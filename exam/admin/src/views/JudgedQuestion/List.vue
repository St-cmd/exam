      ...
      <el-table-column fixed="right" label="操作" width="180">
        <template slot-scope="scope">
          <el-button
            type="text"
            size="small"
            @click="$router.push(`/admin/judgedQuestion/edit/${scope.row._id}`)"
          >编辑</el-button>
...
export default {
  ...
  methods: {
    async fetch () {
      const res = await this.$http.get('/judgedQuestion')
      console.log(res)
      this.items = res.data
    ...
    async remove (row) {
      this.$confirm(`是否删除题目`, '提示', {
        ...
        // 请求删除的接口
        const res = await this.$http.delete(`judgedQuestion/${row._id}`)
        console.log(res)
        this.$message({
          type: 'success',
          message: '删除成功!'
